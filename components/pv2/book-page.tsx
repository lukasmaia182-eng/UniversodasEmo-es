...
export function BookPagePv2({ data }: { data: BookPageData }) {
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-card px-[7%] py-[6%] text-left text-foreground">
      ...
      {/* Pestaña vertical con el nombre de la emoción */}
      <div
      ...
      <Image
        ...
        alt={`Mascota de la emoción ${data.label}`}
        width={120}
        height={120}
      ...
      <div className="mt-auto rounded-md border border-dashed border-foreground/25 bg-foreground/[0.03] p-[5px]">
        <div className="flex items-center gap-1">
          ...
          <span className={`text-[7px] font-extrabold ${data.accentText}`}>Mi turno de escribir</span>
        </div>
        <p className="mt-1 text-[6px] leading-[1.4] text-foreground/70">
          {data.exercise ?? `Cuenta o dibuja un momento en el que sentiste ${data.label.toLowerCase()}.`}
        </p>
